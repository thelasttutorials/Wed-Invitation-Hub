...
import { Heart, Plus, Trash2, ArrowLeft, Save, Check } from "lucide-react";
...
const defaultLoveStory: LoveStoryDraft = { dateLabel: "", title: "", description: "", photoUrl: "" };
...
export default function AdminNewInvitation() {
  ...
  const [loveStory, setLoveStory] = useState<LoveStoryDraft[]>([{ ...defaultLoveStory }]);

  const set = (k: string) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
    setForm(f => ({ ...f, [k]: e.target.value }));

  const createMutation = useMutation({
    mutationFn: (body: any) => apiRequest("POST", "/api/invitations", body),
    ...
    onError: async (err: any) => {
      ...
      toast({ title: body.error || "Gagal membuat undangan", variant: "destructive" });
    },
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createMutation.mutate({ ...form, galleryPhotos, loveStory: validLoveStory });
  };

  const addLoveStoryItem = () => setLoveStory(prev => [...prev, { ...defaultLoveStory }]);
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-30">
        <div className="max-w-3xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-500 text-sm">Buat Undangan</span>
          </div>
          <Link href="/admin">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                <Label htmlFor="slug">URL Undangan (opsional)</Label>
                <Input id="slug" data-testid="input-slug" value={form.slug} onChange={set("slug")} placeholder="cth. budi-sari (otomatis jika kosong)" className="mt-1" />
          ...
          <div className="flex gap-3 pb-8">
            ...
            >
              {createMutation.isPending ? "Menyimpan..." : (
                <><Save className="w-4 h-4 mr-1" /> Simpan Undangan</>
              )}
            </Button>
          </div>
        </form>
